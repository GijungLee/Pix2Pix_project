...
print("Number of train dataset:", len(next(os.walk('./data/facades/train/'))[2]))
print("Number of validation dataset:", len(next(os.walk('./data/facades/val/'))[2]))
print("Number of test dataset:", len(next(os.walk('./data/facades/test/'))[2]))
...
class ImageDataset(Dataset):
    def __init__(self, root, transforms=None, mode="train"):
        ...
        # used test dataset to train model because of small train dataset
        if mode == "train":
    ...
    def __getitem__(self, index):
        ...
        w, h = img.size
        img_A = img.crop((0, 0, w / 2, h)) # left image
        img_B = img.crop((w / 2, 0, w, h)) # right image
...
train_dataset = ImageDataset("./data/facades", transforms=transforms)
val_dataset = ImageDataset("./data/facades", transforms=transforms)
...
# U-Net Down Sampling module
class UNetDown(nn.Module):
    def __init__(self, in_channels, out_channels, normalize=True, dropout=0.0):
        super(UNetDown, self).__init__()
        # w and h is reduced by 2 times
